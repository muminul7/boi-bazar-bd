...
function json(res: PaymentResponse, status: number, body: JsonRecord) {
...
function readHeaderValue(value: HeaderValue): string | null {
...
function resolveClientBaseUrl(req: PaymentRequest, fallback: string): string {
  return (
    normalizePublicBaseUrl(readHeaderValue(req.headers.origin)) ??
    normalizePublicBaseUrl(readHeaderValue(req.headers.referer)) ??
