...
export default function BookDetailPage() {
  ...
  const relatedBooks = allBooks.filter((b) => b.id !== book.id && b.category === book.category).slice(0, 3);
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="container mx-auto py-10">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
          {/* Book Cover — sticky */}
          <motion.div className="lg:col-span-2" initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}>
            <div className="sticky top-24 space-y-5">
              <div className="relative rounded-2xl overflow-hidden shadow-brand-xl group">
                ...
                <div className="absolute top-3 left-3 flex flex-col gap-1.5">
                  {book.bestSeller && (
                    <span className="badge-pill bg-secondary text-secondary-foreground gap-1 flex items-center text-xs shadow-gold"><Award className="h-3 w-3" /> বেস্টসেলার</span>
                  )}
                  {book.newRelease && (
                    <span className="badge-pill bg-primary text-primary-foreground gap-1 flex items-center text-xs"><Zap className="h-3 w-3" /> নতুন</span>
                  )}
                </div>
                {discount > 0 && (
                  <div className="absolute top-3 right-3">
                    <div className="h-12 w-12 rounded-full bg-destructive flex items-center justify-center text-xs font-bold text-destructive-foreground font-body shadow-lg">-{discount}%</div>
                  </div>
                )}
              </div>
              ...
              {book.previewPages.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
                  <Button variant="outline" className="w-full gap-2.5 font-bengali text-base border-primary text-primary hover:bg-primary-subtle py-5 rounded-xl" onClick={() => setPreviewOpen(true)}>
                    <Eye className="h-5 w-5" /> ফ্রি প্রিভিউ পড়ুন ({book.previewPages.length} পৃষ্ঠা)
                  </Button>
                </motion.div>
              )}
              ...
              <div className="bg-card rounded-2xl border border-border p-5 shadow-brand-lg">
                <div className="flex items-baseline gap-3 mb-1">
                  ...
                  {book.originalPrice > book.price && <span className="text-lg text-muted-foreground line-through font-body">৳{book.originalPrice}</span>}
                  {discount > 0 && <span className="badge-pill bg-destructive text-destructive-foreground text-xs font-body">{discount}% ছাড়</span>}
                </div>
                <p className="text-xs text-muted-foreground font-bengali mb-4">একবার কিনুন, আজীবন পড়ুন</p>
                <Button className="w-full gap-2 bg-secondary hover:bg-secondary-light text-secondary-foreground font-bengali text-base mb-3 shadow-gold py-6 rounded-xl" onClick={() => setCheckoutOpen(true)}>
                  <ShoppingCart className="h-5 w-5" /> এখনই কিনুন
                </Button>
          ...
          <div className="lg:col-span-3 space-y-8">
            ...
            {book.description && (
              <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={1} className="relative bg-primary-subtle rounded-2xl p-6 border border-primary/20 overflow-hidden">
            ...
            <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={2} className="grid grid-cols-3 gap-4">
              {[
              ...
              <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={3}>
                ...
                <ul className="space-y-3">
                  {book.outcomes.map((outcome, i) => (
                    <motion.li key={i} className="flex items-start gap-3 bg-card rounded-xl p-3.5 border border-border shadow-brand-sm hover-lift" initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 + i * 0.08 }}>
              ...
              <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={4}>
                ...
                <div className="space-y-3">
                  {book.chapters.map((chapter, i) => (
                    <motion.div key={i} className="bg-card rounded-xl border border-border p-4 hover-lift" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 + i * 0.1 }}>
                      <div className="flex items-center gap-3 mb-2.5">
                        <span className="h-7 w-7 rounded-lg bg-gradient-hero text-primary-foreground text-xs flex items-center justify-center font-bold font-body shrink-0">{i + 1}</span>
            ...
            {/* Guarantee */}
            <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={6} className="bg-gradient-hero rounded-2xl p-6 text-primary-foreground relative overflow-hidden">
              ...
              <div className="relative flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="font-bengali text-sm opacity-90">বই পড়ার পর যদি মনে করেন আপনার কাজে আসেনি, তাহলে ৭ দিনের মধ্যে জানান। সম্পূর্ণ টাকা ফেরত দেওয়া হবে — কোনো প্রশ্ন ছাড়াই।</p>
                </div>
              </div>
            </motion.div>
          </div>
          ...
          <div className="container mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {book.reviews.map((review, i) => (
                <motion.div key={i} className="bg-card rounded-2xl p-5 border border-border shadow-brand-sm hover-lift" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
      ...
      {book.faq.length > 0 && (
        <section className="container mx-auto py-12">
          ...
          <div className="space-y-3 max-w-2xl">
            {book.faq.map((item, i) => (
              <motion.div key={i} className="bg-card rounded-xl border border-border overflow-hidden hover-lift" initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}>
                <button className="w-full flex items-center justify-between p-5 text-left gap-4" onClick={() => setOpenFaqIdx(openFaqIdx === i ? null : i)}>
                  <span className="font-bengali font-semibold text-foreground">{item.question}</span>
                  {openFaqIdx === i ? <ChevronUp className="h-4 w-4 text-primary shrink-0" /> : <ChevronDown className="h-4 w-4 text-muted-foreground shrink-0" />}
                </button>
                {openFaqIdx === i && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} className="px-5 pb-5 border-t border-border">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 lg:hidden bg-card/95 backdrop-blur-md border-t border-border p-3 flex items-center gap-3 shadow-brand-xl">
        <div>
          ...
          {book.originalPrice > book.price && <div className="text-xs text-muted-foreground line-through font-body">৳{book.originalPrice}</div>}
        </div>
        <Button variant="outline" size="sm" className="gap-1.5 font-bengali border-primary text-primary" onClick={() => setPreviewOpen(true)}>
          <Eye className="h-4 w-4" /> প্রিভিউ
        </Button>
        <Button className="flex-1 gap-2 bg-secondary hover:bg-secondary-light text-secondary-foreground font-bengali shadow-gold" onClick={() => setCheckoutOpen(true)}>
      ...
      <BookPreviewReader open={previewOpen} onClose={() => setPreviewOpen(false)} pages={book.previewPages} bookTitle={book.title} totalPages={book.pages} onBuyClick={() => { setPreviewOpen(false); setCheckoutOpen(true); }} />
    </div>
  );
}
